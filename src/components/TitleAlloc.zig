// Autogenerated from http://www.unicode.org/Public/UCD/latest/ucd/UnicodeData.txt by running ucd_gen.
// Placeholders:
//    0. Code point kind
//    1. Struct name
//    2. Array length
//    3. Highest code point
//    4. Lowest code point
//! Unicode Title category code points data.

const std = @import("std");
const mem = std.mem;
const Range = @import("../Range.zig");

const TitleAlloc = @This();

allocator: *mem.Allocator,
array: []bool,
lo: u21 = 453,
hi: u21 = 8188,

pub fn init(allocator: *mem.Allocator) !TitleAlloc {
    var instance = TitleAlloc{
        .allocator = allocator,
        .array = try allocator.alloc(bool, 7736),
    };

    for (instance.array) |*item| {
        item.* = false;
    }

    instance.array[0] = true;
    instance.array[3] = true;
    instance.array[6] = true;
    instance.array[45] = true;
    instance.array[7671] = true;
    instance.array[7687] = true;
    instance.array[7735] = true;

    var index: u21 = 0;
    index = 7619;
    while (index <= 7626) : (index += 1) {
        instance.array[index] = true;
    }
    index = 7635;
    while (index <= 7642) : (index += 1) {
        instance.array[index] = true;
    }
    index = 7651;
    while (index <= 7658) : (index += 1) {
        instance.array[index] = true;
    }

    // Placeholder: 0. Code point kind, 1. Struct name.
    return instance;
}

pub fn deinit(self: *TitleAlloc) void {
    self.allocator.free(self.array);
}

pub fn isTitle(self: TitleAlloc, cp: u21) bool {
    if (cp < self.lo or cp > self.hi) return false;
    const index = cp - self.lo;
    return if (index >= self.array.len) false else self.array[index];
}
